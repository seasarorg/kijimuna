...
public class DtdLoader {
	...
	public static IDtd loadDtd(URL resource) {		
		try {
			...
			throw new DtdException(KijimunaCore.getResourceString(
					"dtd.DtdLoader.1", new Object[] { resource }), e);
		}
	}
	...
	private static void setChildElement(Vector wutkaElements, DefaultDtd dtd) {
		for (Iterator iter = wutkaElements.iterator(); iter.hasNext();) {
			...
			if (item instanceof DTDContainer) {
				...
				for (Iterator it = sequence.getItemsVec().iterator(); it
						.hasNext();) {
	...
	private static void setChildChoiceElement(DefaultDtd dtd, IElementDef element, Object item) {
	...
	private static IElementDef toElement(DTDElement wutkaElement) {
		...
		DefaultElementDef element = new DefaultElementDef(
		        wutkaElement.getName(), empty, pcdata);
		for (Iterator it = wutkaElement.attributes.values().iterator(); it.hasNext();) {
			DTDAttribute wutkaAttribute = (DTDAttribute) it.next();
			DefaultAttributeDef attribute = new DefaultAttributeDef(
					wutkaAttribute.getName(),
					wutkaAttribute.getDecl().name,
					wutkaAttribute.getDefaultValue(),
					toItems(wutkaAttribute.getType())
				);
