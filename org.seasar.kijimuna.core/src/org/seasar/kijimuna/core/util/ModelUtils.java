...
public class ModelUtils implements ConstCore {
	...
	public static String getConstructorDisplay(IRttiConstructorDesctiptor constructor,
			boolean fullDisplay) {
		IRtti rtti = constructor.getParent();
		return getMethodDisplay(rtti, rtti.getQualifiedName(), constructor.getArgs(),
				fullDisplay);
	}

	public static String getMethodDisplay(IRttiInvokableDesctiptor method,
			boolean fullDisplay) {
		return getMethodDisplay(method.getParent(), method.getMethodName(), method
				.getArgs(), fullDisplay);
	}

	public static String getMethodDisplay(IRtti component, String methodName,
			IRtti[] args, boolean fullDisplay) {
	...
	public static boolean hasPropertyElement(IComponentElement component, String name) {
		if (name != null) {
	...
	public static String getInjectedElementName(IRtti component) {
		...
		IDiconElement element = (IDiconElement) component
	...
	public static IRttiMethodDesctiptor[] getImplementMethods(IRtti rtti,
			IRtti implementing) {
		if ((rtti != null) && (implementing != null)) {
			Set set = new TreeSet();
			IRttiMethodDesctiptor[] desc = implementing.getMethods(Pattern.compile(".*"));
			for (int k = 0; k < desc.length; k++) {
				IRttiMethodDesctiptor method = rtti.getMethod(desc[k].getMethodName(),
						desc[k].getArgs(), false);
				set.add(method);
			}
			return (IRttiMethodDesctiptor[]) set.toArray(new IRttiMethodDesctiptor[set
					.size()]);
		}
	...
	public static boolean isAutoBindingProperty(IPropertyElement prop) {
		return prop != null && prop.getChildren().isEmpty()
