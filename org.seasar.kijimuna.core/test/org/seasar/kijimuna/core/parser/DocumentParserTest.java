...
public class DocumentParserTest extends TestCase implements ConstCore{
	...
	private IParseResult executeParser(String filename) throws Exception {
		DocumentHandler handler = new DocumentHandler(new DiconElementFactory(), ID_MARKER_DICONXML, 0, 0);
		...
		try {
			fis = new FileInputStream("test/org/seasar/kijimuna/core/parser/testdata/"+filename);
