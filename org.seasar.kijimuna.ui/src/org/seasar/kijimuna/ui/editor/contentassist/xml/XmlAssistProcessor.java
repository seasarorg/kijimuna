 ...
 */
public class XmlAssistProcessor implements IContentAssistProcessor, ConstUI, XmlConsts {
	...
	protected IContentAssistant getElementClosingContentAssistant(IDtd dtd,
			XmlRegion xmlRegion) {
	...
	private IRegion getAttributeValueRegion(String regionText, int regionOffset,
			int cursorOffset) {
		for (int pos = cursorOffset - 1; pos > -1; pos--) {
			char c = regionText.charAt(pos);
			if (c == '"' || c == '\'') {
	...
	public XmlRegion getXmlRegiton(IDocument doc, int offset) throws BadLocationException {
		...
		if (regionType.equals(TYPE_XML_DECL)) {
			...
			if (cursorOffset == 0) {
				...
				if (lastGt != -1) {
					...
					regionText = stringToOffset.substring(regionOffset, stringToOffset
							.length());
	...
	private String getPrefixFromRegion(XmlRegion xmlRegion) {
		...
		boolean attributeValue = (xmlRegion.getProposalMode() == PROPOSAL_MODE_ATTRIBUTE_VALUE);
		boolean body = (xmlRegion.getProposalMode() == PROPOSAL_MODE_ELEMENT);
		StringBuffer buffer = new StringBuffer();
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int offset) {
		...
		if (assistant != null) {
			ICompletionProposal[] proposals = assistant.getCompletionProposal(prefix,
					offset);
			return proposals;
		}
		return NO_PROPOSALS;
