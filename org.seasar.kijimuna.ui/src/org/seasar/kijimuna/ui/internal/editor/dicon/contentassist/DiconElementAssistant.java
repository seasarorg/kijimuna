...
public class DiconElementAssistant extends XmlElementAssistant implements ConstUI {
    ...
    public DiconElementAssistant(XmlAssistProcessor processor, 
    ...
    private List getComponentProposals(IContainerElement container, String prefix, int offset) {
		...
		for(Iterator it = componentList.iterator(); it.hasNext();) {
		    ...
		    if(StringUtils.existValue(name) && isMatch(name, prefix)) {
		        ICompletionProposal proposal = createProposal(
		                name, name, prefix, offset, name.length(), IMAGE_ICON_COMPONENT);
    ...
    private void addNamespaceProposals(
            List proposals, IContainerElement container, String prefix, int offset) {
		List includes = container.getIncludeList();
		for(int i = 0; i < includes.size(); i++) {
		    IContainerElement child = ((IIncludeElement)includes.get(i)).getChildContainer();
		    if(child != null) {
		        String namespace = child.getNamespace();
		        if(StringUtils.existValue(namespace) && isMatch(namespace, prefix)) {
					proposals.add(createProposal(namespace, namespace,
							prefix, offset, namespace.length(), IMAGE_ICON_CONTAINER));
		        }
    ...
    private List getNamespaceProposals(IContainerElement container, String prefix, int offset) {
		List proposals = new ArrayList();
    ...
    private List getOGNLProposals(IDiconElement element, String prefix, int offset) {
		...
		if(prefix.length() == 0) {
			...
			proposals.add(createProposal(
					item, item, prefix, offset, item.length(), IMAGE_ICON_EXPRESSION));
 		}
 		if(element instanceof IMethodElement) {
 		    String[] items = new String[] { "#err", "#out", "#self" };
 		    for(int i = 0; i < items.length; i++) {
 		        if(isMatch(items[i], prefix)) {
 					proposals.add(createProposal(items[i], items[i], prefix, 
 					        offset, items[i].length(), IMAGE_ICON_EXPRESSION));
 		        }
 		    }
 		}
 		String item = "new ";
 		if(isMatch(item, prefix)) {
			proposals.add(createProposal(item, item, prefix, 
			        offset, item.length(), IMAGE_ICON_EXPRESSION));
 		}
        return proposals;
    }    

    private List getMemberProposals(
		...
		for(int i = 0; i < methods.length; i++) {
		    if(!isStatic || methods[i].isStatic()) { 
		        ...
		        proposals.add(createProposal(replaceStr, methodName, prefix,
		                offset, replaceStr.length() - posDeferencial, IMAGE_ICON_JAVA_METHOD));
		    }
		}
		...
		for(int i = 0; i < fields.length; i++) {
		    if(!isStatic || fields[i].isStatic()) {
		        ...
		        proposals.add(createProposal(replaceStr, fieldName, prefix,
		                offset, replaceStr.length(), IMAGE_ICON_JAVA_FIELD));
		    }
		}
		Collections.sort(proposals, new ProposalComparator());
        return proposals;
	...
	private void addJavaPackageProposals(
	        List proposals, IProject project, String prefix, int offset, String postFix, int postLength) {
		JavaPackageProposalCreator creator = 
		    new JavaPackageProposalCreator(this, project, prefix, offset, postFix, postLength);
		...
		if(lastStack instanceof IDiconElement) {
		    ...
		    if(prefix.startsWith("@")) {
		        ...
		        if(close > 1) {
			        ...
			        proposals.addAll(getMemberProposals(
			                rtti, "@" + className + "@" , member, true, prefix, offset));
		        } else {
		            String packagePrefix = prefix.substring(1);
		            addJavaPackageProposals(
		                    proposals, element.getProject(), packagePrefix, offset, "@", 1);
		        }
		    } else {
		        if(hasNewOperator(prefix)) {
		            // reset super class's list.
		            superProposals.clear();
		            addJavaPackageProposals(proposals, element.getProject(), prefix, offset, "()", 1);
		        } else {
					...
					if(pos > 0) {
					    ...
					    if(el.startsWith("#")) {
							IRtti elRtti;
						    if(element instanceof IMethodElement) {
							    elRtti = execMethodOGNL((IComponentElement)element.getParent(), el);
							    if((elRtti != null) && !(elRtti instanceof HasErrorRtti)) {
							        proposals.addAll(getMemberProposals(
							                elRtti, el + "." , elPrefix, false, prefix, offset));
						        }
						    }
					    } else {
							IRtti elRtti;
						    ...
						    if((elRtti != null) && !(elRtti instanceof HasErrorRtti)) {
						        if(elRtti.getQualifiedName().equals(
									...
									proposals.addAll(getComponentProposals(
									        element.getContainerElement(), prefix, offset));
						        }
						        proposals.addAll(getMemberProposals(
						                elRtti, el + "." , elPrefix, false, prefix, offset));
						    }
						...
						proposals.addAll(getComponentProposals(container, prefix, offset));
						proposals.addAll(getNamespaceProposals(container, prefix, offset));
